...
class About(models.Model):
    journal = models.TextField(
        verbose_name='раздел о журнале',
    )
    editorial_team = models.TextField(
        verbose_name='раздел о редакции',
    )
    contact = models.CharField(
        verbose_name='контакты журнала',
        max_length=300,
    )
...
class Archive(CreatedModel):
    ...
    all_issue_pdf = models.FileField(
    ...
    def get_absolute_url(self):
        return reverse(
            'journal_template:archive_detail', kwargs={'slug': self.pk}
        )
...
class Article(models.Model):
    ...
    def get_absolute_url(self):
        return reverse(
            'journal_template:article_detail', kwargs={'slug': self.pk}
        )
